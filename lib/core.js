 ...
 *
 * Schema v2: items carry id, name, description (+ workflow metadata and, since
 * 0.2.0, an optional agent statusNote/statusAt pair shown on card hover).
...
/** Cap on the agent status note stored per item (hover tooltip text). */
export const STATUS_NOTE_MAX = 400
...
const normItem = (c, i) => {
  ...
  return {
    ...
    statusNote: typeof c.statusNote === 'string' ? c.statusNote.slice(0, STATUS_NOTE_MAX) : null,
    statusAt: strField(c.statusAt, null),
  }
}
...
export function workflowRules(board) {
  ...
  return [
    ...
    '- At the end of EVERY turn, set a short status on your item with kanbanflow_set_status (max 2 sentences: what is done, what is next / what you need from the human). The human sees it on the board card.',
...
export function setStatus(board, { id, note, actor }) {
  const item = board.items.find((i) => i.id === id)
  if (!item) return { error: 'Item not found: ' + id }
  const clean = typeof note === 'string' ? note.trim().slice(0, STATUS_NOTE_MAX) : ''
  item.statusNote = clean || null
  item.statusAt = clean ? new Date().toISOString() : null
  record(board, { itemId: item.id, type: 'item_status', source: actor === 'agent' ? 'agent' : 'human', text: clean || null })
  return { message: clean ? 'Status of ' + item.id + ' updated' : 'Status of ' + item.id + ' cleared' }
}
